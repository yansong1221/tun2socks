...
namespace tuntap {
    ...
    public:
        wintun_service(boost::asio::io_context& ioc)
            : boost::asio::detail::service_base<wintun_service>(ioc), receive_event_(ioc)
        {
        }

        inline void open(const parameter::tun_device& param, boost::system::error_code& ec)
        {
            auto wintun_library = wintun::library::instance(ec);
            if (ec)
                return;

            auto wintun_adapter = wintun_library->create_adapter(param, ec);
            if (ec)
                return;

            wintun_session_ = wintun_adapter->create_session(param, ec);
            if (ec)
                return;
