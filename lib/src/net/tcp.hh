...
namespace net {
    ...
    struct tcp_option
    {
        ...
        enum class option_kind : uint8_t { mss        = 2,
                                           win_scale  = 3,
                                           sack       = 4,
                                           timestamps = 8,
                                           nop        = 1,
                                           eol        = 0 };
        enum class option_len : uint8_t { mss        = 4,
                                          win_scale  = 3,
                                          sack       = 2,
                                          timestamps = 10,
                                          nop        = 1,
    ...
    struct tcp_hdr
    {
        ...
        void write(char* p) const
        {
            ...
            p[13] = (f_fin << 0) | (f_syn << 1) | (f_rst << 2) | (f_psh << 3) | (f_ack << 4) | (f_urg << 5) | (rsvd2 << 6);
    ...
    using packet            = shared_buffer;
    using lowres_clock      = std::chrono::steady_clock;

    class tcp {
    public:
        using ipaddr = boost::asio::ip::address;
        using connid = tcp_endpoint_pair;
        class connection;
        class listener;

        template <typename _class>
        using enable_lw_shared_from_this = std::enable_shared_from_this<_class>;

        template <typename _class>
        using lw_shared_ptr = std::shared_ptr<_class>;
    ...
    private:
        ...
        class tcb : public enable_lw_shared_from_this<tcb> {
            ...
            std::promise<void>                _connect_done;
            std::optional<std::promise<void>> _fin_recvd_promise = std::promise<void>();
            ipaddr                            _local_ip;
            ipaddr                            _foreign_ip;
            ...
            struct send
            {
                ...
                bool                        closed     = false;
                std::promise<void>          _window_opened;
                // Wait for all data are acked
                std::optional<std::promise<void>> _all_data_acked_promise;
                ...
                std::optional<std::promise<void>> _send_available_promise;
            ...
            struct receive
            {
                ...
                std::optional<std::promise<void>> _data_received_promise;
            ...
            tcp_option                _option;
            boost::asio::steady_timer _delayed_ack;
            ...
            boost::asio::steady_timer                  _retransmit;
            boost::asio::steady_timer                  _persist;
        ...
        public:
            ...
            std::future<void> wait_for_data();
            std::future<void> wait_input_shutdown();
            void              abort_reader() noexcept;
            std::future<void> wait_for_all_data_acked();
            std::future<void> wait_send_available();
            std::future<void> send(packet p);
            void              connect();
            packet            read();
            void              close() noexcept;
            void              remove_from_tcbs()
            {
                auto id = connid{address_pair_type(_local_ip, _foreign_ip), _local_port, _foreign_port};
                _tcp._tcbs.erase(id);
            }
        ...
        private:
            ...
            void start_retransmit_timer(clock_type::time_point now)
            {
                ...
                _retransmit.cancel(ec);
                _retransmit.expires_at(tp);
                _retransmit.async_wait([this](boost::system::error_code ec) {
                    if (ec)
                        return;
                    retransmit();
                });
            ...
            void start_persist_timer(clock_type::time_point now)
            {
                ...
                boost::system::error_code ec;
                _persist.cancel(ec);
                _persist.expires_at(tp);
                _persist.async_wait([this](boost::system::error_code ec) {
                    if (ec)
                        return;
                    persist();
                });
            ...
            {
                uint32_t size = 0;
                std::for_each(_snd.data.begin(), _snd.data.end(), [&](unacked_segment& seg) { size += seg.p.size(); });
                return size;
            }
            uint16_t local_mss()
            {
            ...
            {
                _packetq.emplace_back(std::move(p));
            }
        ...
        std::unordered_map<connid, lw_shared_ptr<tcb>> _tcbs;
    ...
    public:
        ...
        std::future<void> poll_tcb(ipaddr to, lw_shared_ptr<tcb> tcb);
    private:
    ...
    void tcp::received(packet p, ipaddr from, ipaddr to)
    {
    ...
    void tcp::send_packet_without_tcb(ipaddr from, ipaddr to, packet p)
    {
    ...
    tcp::tcb::tcb(tcp& t, connid id)
        : _tcp(t), _local_ip(id.local_ip), _foreign_ip(id.foreign_ip), _local_port(id.local_port), _foreign_port(id.foreign_port)
    {
    }

    void tcp::tcb::respond_with_reset(tcp_hdr* rth)
    {
        _tcp.respond_with_reset(rth, _local_ip, _foreign_ip);
    }

    void tcp::respond_with_reset(tcp_hdr* rth, ipaddr local_ip, ipaddr foreign_ip)
    {
        if (rth->f_rst) {
            return;
        }
    ...
    bool tcp::tcb::segment_acceptable(tcp_seq seg_seq, unsigned seg_len)
    {
    ...
    void tcp::tcb::init_from_options(tcp_hdr* th, uint8_t* opt_start, uint8_t* opt_end)
    {
        // Handle tcp options
    ...
    void tcp::tcb::input_handle_listen_state(tcp_hdr* th, packet p)
    {
    ...
    packet tcp::tcb::get_transmit_packet()
    {
    ...
    void tcp::tcb::output_one(bool data_retransmit)
    {
        if (in_state(CLOSED)) {
            return;
        }
    ...
    std::future<void> tcp::tcb::wait_for_data()
    {
        if (!_rcv.data.empty() || foreign_will_not_send()) {
            return make_ready_std::future<void>();
        }
        _rcv._data_received_promise = std::promise<void>();
        return _rcv._data_received_promise->get_future();
    }

    std::future<void> tcp::tcb::wait_input_shutdown()
    {
        if (!_fin_recvd_promise) {
            return make_ready_std::future<void>();
        }
    ...
    std::future<void> tcp::tcb::wait_for_all_data_acked()
    {
        if (_snd.data.empty() && _snd.unsent_len == 0) {
            return make_ready_std::future<void>();
        }
        _snd._all_data_acked_promise = std::promise<void>();
        return _snd._all_data_acked_promise->get_future();
    }

    void tcp::tcb::connect()
    {
    ...
    std::future<void> tcp::tcb::wait_send_available()
    {
        if (_snd.max_queue_space > _snd.current_queue_space) {
            return make_ready_std::future<void>();
        }
        _snd._send_available_promise = std::promise<void>();
        return _snd._send_available_promise->get_future();
    }

    std::future<void> tcp::tcb::send(packet p)
    {
        // We can not send after the connection is closed
        if (_snd.closed || in_state(CLOSED)) {
            return make_exception_std::future<void>(tcp_reset_error());
        }
    ...
    bool tcp::tcb::should_send_ack(uint16_t seg_len)
    {
        ...
        _delayed_ack.expires_after(200ms);
        _delayed_ack.async_wait([this](boost::system::error_code ec) {
            if (ec)
                return;
    ...
    void tcp::tcb::clear_delayed_ack() noexcept
    {
        _delayed_ack.cancel();
    }

    bool tcp::tcb::merge_out_of_order()
    {
        bool merged = false;
    ...
    void tcp::tcb::insert_out_of_order(tcp_seq seg, packet p)
    {
        _rcv.out_of_order.merge(seg, std::move(p));
    }

    void tcp::tcb::trim_receive_data_after_window()
    {
        abort();
    }

    void tcp::tcb::persist()
    {
    ...
    void tcp::tcb::update_rto(clock_type::time_point tx_time)
    {
    ...
    void tcp::tcb::update_cwnd(uint32_t acked_bytes)
    {
        uint32_t smss = _snd.mss;
    ...
    tcp_seq tcp::tcb::get_isn()
    {
        return make_seq(0);
    }

    std::optional<packet> tcp::tcb::get_packet()
    {
        _poll_active = false;
        if (_packetq.empty()) {
            output_one();
