...
public:
    ...
    inline static tcp_pcb *lwip_tcp_new() { return ::tcp_new(); }

    inline static udp_pcb *lwip_udp_new() { return ::udp_new(); }
    ...
    inline static tcp_pcb *lwip_tcp_listen(tcp_pcb *pcb) { return ::tcp_listen(pcb); }
    ...
    inline static void lwip_tcp_arg(tcp_pcb *pcb, void *arg) { return tcp_arg(pcb, arg); }

    inline static void lwip_tcp_receive(
        struct tcp_pcb *pcb, std::function<std::remove_pointer<tcp_recv_fn>::type> receive)
    {
        if (receive == NULL) {
            tcp_err(pcb, NULL);
        } else {
    ...
    inline static void lwip_tcp_accept(
        struct tcp_pcb *pcb, std::function<std::remove_pointer<tcp_accept_fn>::type> accept)
    {
    ...
    inline static void lwip_udp_timeout(
        struct udp_pcb *pcb, std::function<std::remove_pointer_t<udp_timeout_fn>> timeout_fn)
    {
    ...
    inline static void lwip_udp_disconnect(struct udp_pcb *pcb) { ::udp_disconnect(pcb); }

    inline static void lwip_udp_remove(struct udp_pcb *pcb) { return udp_remove(pcb); }
    ...
    inline static u32_t lwip_tcp_sndbuf(tcp_pcb *pcb) { return tcp_sndbuf(pcb); }

    inline static err_t lwip_tcp_output(tcp_pcb *pcb) { return ::tcp_output(pcb); }
    ...
    inline err_t lwip_ip_input(pbuf *p) { return loopback_->input(p, loopback_); }
